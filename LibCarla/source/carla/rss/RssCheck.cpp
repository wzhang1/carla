...
// constants for deg-> rad conversion PI / 180
  ...
  RssCheck::RssCheck() {
    mRssSituationExtraction = std::make_shared<::ad_rss::core::RssSituationExtraction>();
    mRssSituationChecking = std::make_shared<::ad_rss::core::RssSituationChecking>();
    mRssResponseResolving = std::make_shared<::ad_rss::core::RssResponseResolving>();

    mEgoVehicleDynamics = std::make_shared<::ad_rss::world::RssDynamics>();
    mOtherVehicleDynamics = std::make_shared<::ad_rss::world::RssDynamics>();
  ...
  ::ad_rss::world::Velocity &rssEgoVelocity, bool visualizeResults) {
    ...
    carla::geom::Location egoVehicleLocation = carlaEgoVehicle->GetLocation();
    ...
    if (egoClientWaypoint != nullptr) {
      ...
      auto &map = clientMap->GetMap();
      auto egoWaypoint = map.GetWaypoint(egoVehicleLocation);
      auto &egoLane = map.GetLane(*egoWaypoint);
      ...
      for (auto &laneSection : egoRoad->GetLaneSections()) {
        for (const auto &pair : laneSection.GetLanes()) {
          ...
          static_cast<uint32_t>(carla::road::Lane::LaneType::Driving)) > 0) {
            ...
            // TODO: handle multi segments

            auto laneLength = lane.GetLength();
            // evaluate width at lane start
            // TODO: evaluate at several positions
            ...
            // TODO: use more sofisticated transformation for the lane model
            laneSegment.length.minimum = ::ad_rss::physics::Distance(laneLength);
            laneSegment.length.maximum = ::ad_rss::physics::Distance(laneLength);
            laneSegment.width.minimum = ::ad_rss::physics::Distance(laneWidth);
            laneSegment.width.maximum = ::ad_rss::physics::Distance(laneWidth);
      ...
      calculateOccupiedRegions(egoBounds, egoVehicle.occupiedRegions, map, drivingInRoadDirection,
      egoVehicleTransform);
      ...
      for (const auto &actor : *vehicles) {
        ...
        if (egoClientWaypoint->GetRoadId() == otherVehicleClientWaypoint->GetRoadId()) {
          ...
          calculateOccupiedRegions(otherBounds, otherVehicle.occupiedRegions, map, drivingInRoadDirection,
      ...
      bool result = mRssSituationExtraction->extractSituations(worldModel, situationSnapshot);

      ::ad_rss::state::RssStateSnapshot stateSnapshot;
      if (result) {
        result = mRssSituationChecking->checkSituations(situationSnapshot, stateSnapshot);
      }
      if (result) {
        result = mRssResponseResolving->provideProperResponse(stateSnapshot, response);
      }
      if (result) {
  ...
  void RssCheck::calculateLatLonVelocities(const carla::geom::Transform &laneLocation,
  const carla::geom::Vector3D &velocity, ::ad_rss::world::Object &rssObject,
...
// return front right, back right, back left, front left bounds
  std::array<carla::geom::Location,
  4u> RssCheck::getVehicleBounds(const carla::client::Vehicle &vehicle) const {
    ...
    carla::geom::Location frontExtent{cosine *box.extent.x, sine *box.extent.x, 0.0f};
    carla::geom::Location rightExtent{-sine * box.extent.y, cosine *box.extent.y, 0.0f};
  ...
  void RssCheck::calculateOccupiedRegions(const std::array<carla::geom::Location, 4u> &bounds,
  ::ad_rss::world::OccupiedRegionVector &occupiedRegions,
  const carla::road::Map &map, const bool drivingInRoadDirection,
  const carla::geom::Transform &vehicleTransform) const {
    bool regionValid = true;
    for (const auto &bound : bounds) {
      auto boundWaypoint = map.GetWaypoint(bound);
      ...
      auto &lane = map.GetLane(*boundWaypoint);
      auto road = lane.GetRoad();
      uint64_t laneSegmentId = calculateLaneSegmentId(map.GetLane(*boundWaypoint));
      ...
      auto waypointLocation = map.ComputeTransform(*boundWaypoint);
      ...
      auto width = map.GetLaneWidth(*boundWaypoint);
      ...
      if (regionIt == occupiedRegions.end()) {
        ...
        newRegion.lonRange.maximum = ::ad_rss::physics::ParametricValue(lon);
        newRegion.latRange.minimum = ::ad_rss::physics::ParametricValue(lat);
        newRegion.latRange.maximum = ::ad_rss::physics::ParametricValue(lat);
  ...
  const carla::geom::Transform &egoVehicleTransform,
  carla::client::World &world) const {
    ...
    for (std::size_t i = 0; i < stateSnapshot.individualResponses.size(); ++i) {
      ...
      carla::SharedPtr<carla::client::ActorList> vehicleList =
      world.GetActors(std::vector<carla::rpc::ActorId>{vehicleId});
      carla::geom::Location egoPoint = egoVehicleLocation;
      egoPoint.z += 0.05f;
      ...
      carla::geom::Location lineOffset{-sine * 0.1f, cosine * 0.1f, 0.0f};
      for (const auto &actor : *vehicleList) {
        const auto vehicle = boost::dynamic_pointer_cast<carla::client::Vehicle>(actor);
        carla::geom::Location point = vehicle->GetLocation();
        point.z += 0.05f;
        carla::sensor::data::Color lonColor{0u, 255u, 0u};
        carla::sensor::data::Color latLColor = lonColor;
        carla::sensor::data::Color latRColor = lonColor;
        carla::sensor::data::Color indicatorColor = lonColor;
        bool dangerous = ::ad_rss::state::isDangerous(state);
        if (dangerous) {
          indicatorColor = carla::sensor::data::Color{255u, 0u, 0u};
        }
